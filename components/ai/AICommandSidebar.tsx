...
export const AICommandSidebar = ({ isOpen, onClose, initialPrompt = '', width }: { isOpen: boolean, onClose: () => void, initialPrompt?: string, width: number }) => {
    ...
    >
      <div className="p-6 border-b border-slate-800 flex justify-between items-center">
        <div className="flex items-center gap-3">
          ...
          <h3 className="text-lg font-bold text-white">AI Command</h3>
        </div>
        <button onClick={onClose} className="text-slate-500 hover:text-white p-1 hover:bg-slate-800 rounded-md transition-colors">
          <X size={20} />
        </button>
      </div>
        ...
        <div className="space-y-2">
          ...
          <div className="relative">
            <textarea
              ...
              className="w-full bg-slate-800 border border-slate-700 rounded-2xl p-4 text-white placeholder:text-slate-600 focus:outline-none focus:ring-2 focus:ring-indigo-500/50 min-h-[150px] resize-none text-sm"
            />
            <button
